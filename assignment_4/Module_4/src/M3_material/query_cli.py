...
def main():
    ...
    print(f"American applicants in the 2026 cohort have an average GPA of {avg_gpa_amer}.")

    # Question 5
    accept_pct_2026 = acceptance_rate_fall_2026(True)
    print(f"{accept_pct_2026}% of 2026 cohort applications resulted in acceptances.")

    # Question 6
    avg_gpa_accepted = avg_gpa_acceptances_fall_2026(True)
    print(f"Accepted applicants in the 2026 cohort have an average GPA of {avg_gpa_accepted}.")

    # Question 7
    jhu_cs_ms = count_jhu_masters_cs(True)
    print(
        "There are {0} application entries from students who applied to "
        "Johns Hopkins University for a master’s degree in Computer Science.".format(jhu_cs_ms)
    )
