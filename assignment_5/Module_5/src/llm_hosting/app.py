...
# pylint: disable=global-statement,broad-exception-caught,consider-using-with
...
def _read_lines(path: str) -> List[str]:
    """Read non-empty lines from a file into a list."""
    try:
...
def _cli_process_file(in_path: str, out_path: str | None, append: bool, to_stdout: bool) -> None:
    ...
    with open(in_path, "r", encoding="utf-8") as file_handle:
        rows = _normalize_input(json.load(file_handle))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Output path for JSON Lines (ndjson). Defaults to <input>.jsonl "
            "when --file is set."
        ),
    )
    parser.add_argument(
