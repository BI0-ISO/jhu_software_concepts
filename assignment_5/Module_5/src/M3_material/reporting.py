...
# pylint: disable=line-too-long,too-many-locals,too-many-statements,consider-using-f-string,invalid-name
...
def _db_hardening_lines():
    """Return lines describing least-privilege database permissions."""
    lines = []
    lines.append("")
    lines.append("Database Hardening (Least Privilege)")
    lines.append("-" * len("Database Hardening (Least Privilege)"))
    lines.extend(_wrap("Permissions granted:"))
    lines.extend(_wrap("- CONNECT on the database (to allow app access)."))
    lines.extend(_wrap("- USAGE on schema public (to access tables)."))
    lines.extend(_wrap("- SELECT, INSERT, UPDATE on applicants and pull_jobs (app reads and writes these tables)."))
    lines.extend(_wrap("- No DROP/ALTER/OWNER permissions granted."))
    lines.append("")
    lines.extend(_wrap("Example SQL snippet:"))
    lines.extend(_wrap("CREATE USER app_user WITH PASSWORD 'change-me';"))
    lines.extend(_wrap("GRANT CONNECT ON DATABASE your_db_name TO app_user;"))
    lines.extend(_wrap("GRANT USAGE ON SCHEMA public TO app_user;"))
    lines.extend(_wrap("GRANT SELECT, INSERT, UPDATE ON TABLE applicants, pull_jobs TO app_user;"))
    lines.extend(_wrap("ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE ON TABLES TO app_user;"))
    return lines


def generate_db_hardening_report(path):
    """Generate a standalone PDF documenting least-privilege DB access."""
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    lines = []
    lines.extend(_wrap("Database Hardening Report"))
    lines.extend(_wrap(f"Generated: {now}"))
    lines.extend(_db_hardening_lines())
...
def generate_pdf_report(results, path):
    ...
    lines.extend(_db_hardening_lines())
    lines.append("")
    section("Packaging (setup.py)")
    lines.extend(_wrap(
        "Packaging matters because it makes the project installable and keeps "
        "imports consistent across local runs, tests, and CI."
    ))
    lines.extend(_wrap(
        "A setup.py enables editable installs (pip install -e .), which reduces "
        "path issues and \"it works on my machine\" problems."
    ))
    lines.extend(_wrap(
        "Tools like uv can also read setup metadata to reproduce environments "
        "reliably from the same source."
    ))
