...
def parse_date(value):  # pylint: disable=too-many-return-statements
...
def normalize_record(r: dict) -> dict:
    ...
    llm_program = clean_text(
        r.get("llm-generated-program") or r.get("llm_generated_program")
    )
    llm_university = clean_text(
        r.get("llm-generated-university") or r.get("llm_generated_university")
    )
    ...
    year = extract_year(r.get("semester_year_start")) or (
        date_added[:4] if date_added else None
    )
    ...
    return {
        ...
        "us_or_international": clean_text(
            r.get("citizenship") or r.get("us_or_international")
        ),
        ...
        "degree": clean_text(
            r.get("degree_type") or r.get("masters_or_phd") or r.get("degree")
        ),
