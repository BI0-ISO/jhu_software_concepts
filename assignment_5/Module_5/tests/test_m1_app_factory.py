...
def test_m1_create_app_fallback_blueprint(monkeypatch):
    # Force the legacy Module_3 import to fail so the fallback path runs.
    monkeypatch.setitem(__import__("sys").modules, "Module_3", None)
    monkeypatch.setitem(__import__("sys").modules, "Module_3.board", None)

    app = M1_material.create_app()

    # The local Module 1 blueprint should be registered by name.
    assert "m1_pages" in app.blueprints
